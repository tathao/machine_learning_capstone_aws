...
def main(args):
    ...
    valid_dataset_directory = os.environ.get("SM_CHANNEL_VALIDATION", "/opt/ml/input/data")
    ...
    for epoch in range(1, args.num_epochs+1):
        model = train(model, data_loaders["train"], loss_criterion, optimizer, epoch, hook ,device)
    
        '''
        Test the model to see its accuracy
        '''
        test(model, data_loaders["test"], loss_criterion, hook ,device)
